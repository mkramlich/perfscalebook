...
items = [
    ...
    (0,   "elastic microservices arch for greater scalability, resiliency"),
    (0,   "uniform distribution of tasks across a regular time window where possible (eg. say you have 100 servers which must be rebooted one per day (and a server reboot takes typically max of 2 minutes), you don’t reboot them all at the same moment, but spread them out evenly so never more than 1 server rebooting at any point during any 24 hour window) NOTE this is very close to existing #94 and 93"),
    (0,   "random jigging/jittering of attempt tries to avoid stampedes"),
    (0,   "assembly. inline ASM (like in C)"),
    (0,   "add some cloud, NN, ML and 'data science' specific items to candidate list, where distinction warrants"),
    (0,   "TODO some in my notes on phone?"),
    (0,   "speculative branch prediction (similar to 17 Precompute Predicted Requests)"),
    (0,   "probable bets (needs work; flesh out or drop this)"),
    ...
    (0,   "dedicated physical network routes (ie. what Google and Amazon likely do between their data centers)"),
    (0,   "mass coordination by clients/users to self-distribute target addresses and request times in a balance way (redund?)"),
    (0,   "JIT bytecode optimizations"),
    (0,   "in GUI, not rerender to pixels any faster than the display refresh cycle"),
    (0,   "in 3D UIs, view-model culling"),
    (0,   "image compression (to reduce bandwidth, boost throughput, reduce disk space footprint). add audio and video (similar to 34 Compression of Large Transfer Payloads; related to 102)"),
    (0,   "at runtime drop value checking guards (think asserts and null guards) to boost speed, if wont cause problems, or tradeoff on elavated error rate is acceptable"),
    (0,   "disable GC (I believe already have 'tune GC' in my list)"),
    (0,   "log output reducing (eg. via quieter levels) or total disabling/muting"),
    (0,   "log writes to faster write-turnaround target (even if ephemeral like memory)"),
    (0,   "no alloc function calls (so caller can reuse pre-allocated memory slabs, to reduce latency and better control lifecycle of memory objects) (maybe redund/overlap with existing 'object pools' item)"),
    (0,   "ensure no memory gets paged out to disk (via OS-wide disabling, or, by having the app workoad be very careful?) {related to 42 Tuning OS Params)"),
    (0,   "faked not real (eg. mocked-up screen flows in a demo, not working code yet)"),
    (0,   "that neat technique Carmack once tweeted about? (IIRC)"),
    (0,   "smaller IC chips (very speculative and may drop this; TBD cuz on one hand I see why latencies within-chip could be faster however may also be other physics/materials effects at that tiny scale which disrupt or push back in opposite direction)"),
    (0,   "more on 1 chip (instead of external GPU or NN compute; eg. SoC, Apple's M1) (can't believe it took me 3+ years to add this)"),
]
